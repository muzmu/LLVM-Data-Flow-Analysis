...
namespace llvm {
    ...
    BitVector* iterative_model::run_analysis(){
        ...
        if(fwd){
            ...
            BasicBlock* starting_point=&*func->getBasicBlockList().rbegin();
            BitVector* input2;
            BitVector* input= state[starting_point]->input;
            while(!converged){
                for(auto FI=func->getBasicBlockList().rbegin(), FE=func->getBasicBlockList().rend(); FI!=FE; ++FI)
                {
                    converged=false;
                    BasicBlock* block = &*FI;
                    state[block]->output = meet(block,state); 
                    input2 = transfer_function(block,state);
                    if(input == input2){
                        converged = true;
                    }
                    state[block]->input=input2;
                }
            }
        }
